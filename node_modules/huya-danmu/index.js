...
class huya_danmu extends events {
    ...
    async _get_chat_info() {
        try {
            ...
            let info = {}
            let m = body.match(/window.HNF_GLOBAL_INIT\s*=\s*(\{.+?\})\s*<\/script>/)
            if (!m) return
            let j = JSON.parse(m[1])
            let yyuid = j.roomInfo.tProfileInfo.lUid
            let topsid = j.roomInfo.tLiveInfo.lChannel || j.roomInfo.tRecentLive.lChannel || yyuid
            let subsid = j.roomInfo.tLiveInfo.lLiveChannel || j.roomInfo.tRecentLive.lLiveChannel || yyuid
            ...
            this.emit('error', new Error('Fail to get info: ' + e.message))
        }
    }
